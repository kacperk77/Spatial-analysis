...
join3 <- join3 %>% select(geo, emp_share)
...
model.liniowy <- lm(spatial_data2$robbery ~ spatial_data2$gdp_per_capita+spatial_data2$pop_density+
                      spatial_data2$emp_share)
...
moran.test(res, W2_list) #Test statistic with p-value
...
spatial_data2$podzial <- ifelse(spatial_data2$robbery < 200, 1, 0)
