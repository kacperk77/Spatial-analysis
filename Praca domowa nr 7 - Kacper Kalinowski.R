...
SAR <- lagsarlm(spatial_data3$robbery ~  spatial_data3$gdp_per_capita+ 
                  spatial_data3$pop_density + spatial_data3$n_of_police_per_100k+
                  spatial_data3$emp_share, listw = W2_list, control = list(fdHess = TRUE))
impacts.SAR <- impacts(SAR, listw = W2_list, zstats = TRUE, R = 200)
summary(impacts.SAR)
...
#Let us focus on one regressor: wages 
#METHOD DELTA
b_pop <- SAR$coefficients["spatial_data3$pop_density"]
S.b_pop <- M * b_pop

#wybranie pierwszego regionu
spatial_data3$effects_first_region <- S.b_pop[,1]
...
pal <- colorRampPalette(c("red", "white", green_area), bias = 0)

spplot(spatial_data3, zcol = "effects_first_region", colorkey = TRUE,  col.regions = pal(100), cuts = 99,
       par.settings = list(axis.line = list(col =  'transparent')),
       main = "Wp³yw jednostkowej zmiany pop_density w regionie Miasto Lodz na zmienn¹
objaœnian¹ w poszczególnych regionach")
